...
class DBF1RaceAnalyzer:
    ...
    def get_pit_stop_data(self, event_name, year, driver):
        ...
        SELECT 
            lap_number,
            MIN(tire_compound) as new_compound
        FROM 
            telemetry t
        JOIN 
            driver d ON t.driver_id = d.driver_id
        ...
        WHERE 
            ...
            AND d.driver_code = %s
            AND s.session_type = 'R'
            AND (LAG(tire_compound) OVER (ORDER BY lap_number) != tire_compound 
                OR LAG(tire_compound) OVER (ORDER BY lap_number) IS NULL)
        GROUP BY 
            lap_number
        ORDER BY 
            lap_number
        ...
        for driver in drivers:
            ...
            for compound in compounds_summary:
                if compounds_summary[compound]['laps'] > 0:
                    ...
                    compounds_summary[compound]['avg_degradation'] = compounds_summary[compound]['degradation_sum'] / compounds_summary[compound]['stints']
            ...
            for compound in common_compounds:
                ...
                if len(driver_perf) > 1:
                    ...
                    for i, (driver, data) in enumerate(sorted_drivers):
                        if i == 0:
                            insights.append(f"* {driver} managed the {compound} tires better with {data['avg_degradation']:.4f}s/lap degradation")
                        else:
    ...
    def save_to_database(self, event_name, year, drivers, insights, lap_times_path, tire_strategy_path, position_path, tire_data):
        if not self.conn:
            self.connect()
    ...
    def run_race_analysis(self, event_name, year, drivers, save_to_db=False):
        ...
        base_path = os.path.join("analysis", "race_analysis", event_name, f"{'-'.join(drivers)}")
        os.makedirs(base_path, exist_ok=True)
        
        output_file = os.path.join(base_path, f"race_analysis_db.md")
        lap_times_path = os.path.join(base_path, "lap_times_db.png")
        tire_strategy_path = os.path.join(base_path, "tire_strategy_db.png")
        position_path = os.path.join(base_path, "position_changes_db .png")
