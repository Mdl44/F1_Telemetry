...
class DBF1QualifyingAnalyzer:
    ...
    def generate_quali_insights(self, event_name, year, drivers):
        ...
        for driver, laps in all_laps.items():
            ...
            print(
                f"Found {len(comparable_laps[driver])} comparable laps (within 1s total and sector times) for {driver}")
        ...
        if len(fastest_laps) == 2:
            ...
            for driver, lap in fastest_laps.items():
                sector_table.append(
                    f"|{driver}|{lap['sector1_time']:.3f}s|{lap['sector2_time']:.3f}s|{lap['sector3_time']:.3f}s|")
    ...
    def create_delta_plot(self, event_name, year, drivers, save_path=None):
        ...
        for driver in drivers:
            cursor = self.conn.cursor()
            cursor.execute("""
                           SELECT MIN(NULLIF(sector1_time, 0)) as s1_time,
                           ...
                           WHERE e.event_name LIKE %s
                             ...
                             AND d.driver_code = %s
                             AND s.session_type = 'Q'
                             AND lap_number = (SELECT lap_number
                                               FROM telemetry
    ...
    def run_quali_analysis(self, event_name, year, drivers, save_to_db=True, output_dir=None):
        ...
        if existing_analysis:
            print(
                f"Found existing analysis (ID: {existing_analysis['analysis_id']}) for {drivers[0]} vs {drivers[1]} at {event_name} {year}")
            print(f"Using existing analysis for {drivers[0]} vs {drivers[1]} at {event_name} {year}")
        ...
        for driver in drivers:
            ...
            if driver_laps is not None and not driver_laps.empty:
                ...
                if len(all_laps[driver]) > 0:
                    print(
                        f"  Lap times range: {all_laps[driver]['lap_time'].min():.3f}s - {all_laps[driver]['lap_time'].max():.3f}s")
            else:
    ...
    def save_to_database(self, event_name, year, drivers, insights, speed_viz_path, delta_viz_path):
        ...
        try:
            ...
            cursor.execute("""
                           ...
                           VALUES (%s, %s, %s, %s, %s, %s, %s) RETURNING analysis_id
                           """, (
                               session_id,
                               'qualifying',
                               event_name,
